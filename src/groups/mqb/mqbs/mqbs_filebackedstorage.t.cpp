...
MockDataStore(int partitionId): d_id(0) {
    d_config.setPartitionId(partitionId);
}

virtual int writeMessageRecord(
    ...
    {     
        ...
        auto recOffset = d_id;
        auto* iter = reinterpret_cast<mqbs::DataStoreConfig::RecordIterator*>(handle);
        // auto& hhandle = *reinterpret_cast<Item**>(handle);
        // Item* item_p = new Item(mqbs::DataStoreRecordKey(sequenceNum, primaryLeaseId), mqbs::DataStoreRecord(recType, recOffset));
        // hhandle = item_p;

        bsl::pair<mqbs::DataStoreConfig::RecordIterator, bool> insertRc = d_records.insert(bsl::make_pair(mqbs::DataStoreRecordKey(sequenceNum, primaryLeaseId), mqbs::DataStoreRecord(recType, recOffset)));
        *iter = insertRc.first;

        cerr << "writeMessageRecord " << d_id << " " << attributes->arrivalTimepoint() << endl;
...
virtual void loadMessageAttributesRaw(
    mqbi::StorageMessageAttributes* buffer,
    const mqbs::DataStoreRecordHandle&    handle) const {
    //     cerr << "loadMessageAttributesRaw sizeof(Item) = " << sizeof(Item) << endl;
    //     cerr << "loadMessageAttributesRaw sizeof(DataStoreRecordHandle) = " << sizeof(mqbs::DataStoreRecordHandle) << endl;
    //     cerr << "loadMessageAttributesRaw sizeof(handle) = " << sizeof(handle) << endl;

        // const auto& item = *reinterpret_cast<const mwcc::OrderedHashMap_SequentialIterator<Item>*>(&handle);
        // const Item* item = reinterpret_cast<const Item*>(recit->);
         
        // const Item** item = reinterpret_cast<const Item**>(&handle);
        const auto& iter = *reinterpret_cast<const  mqbs::DataStoreConfig::RecordIterator*>(&handle);
        auto id = iter->second.d_recordOffset;
        // cerr << "loadMessageAttributesRaw item addr = " << (item) << endl;
        cerr << "loadMessageAttributesRaw recOffset = " << id << endl;
        // cerr << "loadMessageAttributesRaw recType = " << item->second.d_recordType << endl;
        *buffer = d_attributes.at(id);
}

virtual void loadMessageRaw(
    ...
    const mqbs::DataStoreRecordHandle&    handle) const 
{
    ...
    // const auto& item = *reinterpret_cast<const mwcc::OrderedHashMap_SequentialIterator<Item>*>(&handle);
    // const mqbs::DataStoreConfig::RecordIterator& recit = *reinterpret_cast<const mqbs::DataStoreConfig::RecordIterator*>(&handle);
    // const Item* item = reinterpret_cast<const Item*>(recit->);
    
    const auto& iter = *reinterpret_cast<const mqbs::DataStoreConfig::RecordIterator*>(&handle);
    auto id = iter->second.d_recordOffset;
    // cerr << "loadMessageAttributesRaw item addr = " << (item) << endl;
    cerr << "loadMessageRaw " << id << endl;

    *appData = d_appData.at(id);
    *options = d_options.at(id);
}

virtual BloombergLP::mqbi::Dispatcher* dispatcher() {return nullptr;}
mqbi::DispatcherClientData d_dispatcherClientData;
virtual BloombergLP::mqbi::DispatcherClientData& dispatcherClientData() {return d_dispatcherClientData;}
virtual void onDispatcherEvent(const BloombergLP::mqbi::DispatcherEvent&) {}
virtual void flush() {}
virtual const BloombergLP::mqbi::Dispatcher* dispatcher() const {return nullptr;}
virtual const BloombergLP::mqbi::DispatcherClientData& dispatcherClientData() const {return d_dispatcherClientData;}
bsl::string d_description; 
virtual const bsl::string& description() const {return d_description;}

virtual int open(const QueueKeyInfoMap&) {return 0;}
virtual void close(bool) {}
virtual void createStorage(bsl::shared_ptr<BloombergLP::mqbs::ReplicatedStorage>*, const BloombergLP::bmqt::Uri&, const BloombergLP::mqbu::StorageKey&, BloombergLP::mqbi::Domain*) {}
virtual int writeQueueCreationRecord(BloombergLP::mqbs::DataStoreRecordHandle*, const BloombergLP::bmqt::Uri&, const BloombergLP::mqbu::StorageKey&, const AppIdKeyPairs&, BloombergLP::bsls::Types::Uint64, bool) {return 0;}
virtual int writeQueuePurgeRecord(BloombergLP::mqbs::DataStoreRecordHandle*, const BloombergLP::mqbu::StorageKey&, const BloombergLP::mqbu::StorageKey&, BloombergLP::bsls::Types::Uint64) {return 0;}
virtual int writeQueueDeletionRecord(BloombergLP::mqbs::DataStoreRecordHandle*, const BloombergLP::mqbu::StorageKey&, const BloombergLP::mqbu::StorageKey&, BloombergLP::bsls::Types::Uint64) {return 0;}
virtual int writeConfirmRecord(BloombergLP::mqbs::DataStoreRecordHandle*, const BloombergLP::bmqt::MessageGUID&, const BloombergLP::mqbu::StorageKey&, const BloombergLP::mqbu::StorageKey&, BloombergLP::bsls::Types::Uint64, BloombergLP::mqbs::ConfirmReason::Enum) {return 0;}
virtual int writeDeletionRecord(const BloombergLP::bmqt::MessageGUID&, const BloombergLP::mqbu::StorageKey&, BloombergLP::mqbs::DeletionRecordFlag::Enum, BloombergLP::bsls::Types::Uint64) {return 0;}
virtual int writeSyncPointRecord(const BloombergLP::bmqp_ctrlmsg::SyncPoint&, BloombergLP::mqbs::SyncPointType::Enum) {return 0;}
virtual int removeRecord(const BloombergLP::mqbs::DataStoreRecordHandle&) {return 0;}
virtual void removeRecordRaw(const BloombergLP::mqbs::DataStoreRecordHandle&) {}
virtual void processStorageEvent(const bsl::shared_ptr<BloombergLP::bdlbb::Blob>&, bool, BloombergLP::mqbnet::ClusterNode*) {}
virtual int processRecoveryEvent(const bsl::shared_ptr<BloombergLP::bdlbb::Blob>&) {return 0;}
virtual void processReceiptEvent(unsigned int, BloombergLP::bsls::Types::Uint64, BloombergLP::mqbnet::ClusterNode*) {}
virtual int issueSyncPoint() {return 0;}
virtual void setPrimary(BloombergLP::mqbnet::ClusterNode*, unsigned int) {}
virtual void clearPrimary() {}
virtual void dispatcherFlush(bool, bool) {}
virtual bool isOpen() const {return true;}
virtual const BloombergLP::mqbs::DataStoreConfig& config() const {return d_config;}
virtual unsigned int clusterSize() const {return 1u;}
virtual BloombergLP::bsls::Types::Uint64 numRecords() const {return d_attributes.size();}
virtual void loadMessageRecordRaw(BloombergLP::mqbs::MessageRecord*, const BloombergLP::mqbs::DataStoreRecordHandle&) const {}
virtual void loadConfirmRecordRaw(BloombergLP::mqbs::ConfirmRecord*, const BloombergLP::mqbs::DataStoreRecordHandle&) const {}
virtual void loadDeletionRecordRaw(BloombergLP::mqbs::DeletionRecord*, const BloombergLP::mqbs::DataStoreRecordHandle&) const {}
virtual void loadQueueOpRecordRaw(BloombergLP::mqbs::QueueOpRecord*, const BloombergLP::mqbs::DataStoreRecordHandle&) const {}
virtual unsigned int getMessageLenRaw(const BloombergLP::mqbs::DataStoreRecordHandle&) const {return sizeof(int);}
virtual unsigned int primaryLeaseId() const {return 0;}
virtual bool hasReceipt(const BloombergLP::mqbs::DataStoreRecordHandle&) const {return false;}
};
...
// TODO: or rely on base class ReplicatedStorage
struct Tester {
  private:
    // PRIVATE TYPES
    typedef mqbs::DataStoreConfig::Records Records;
  ...
  public:
    ...
    , d_allocator_p(allocator)
    {
        ...
        d_fileBackedStorage_mp.load(new (*d_allocator_p) mqbs::FileBackedStorage(
                                      &d_dataStore_p,
    ...
    mqbs::FileBackedStorage& storage() { return *d_fileBackedStorage_mp.ptr(); }
...
//
// Concerns:
//   A 'mqbs::FileBackedStorage' implements, but does not adhere to, some
...
//       protocol are also provided in 'mqbs::FileBackedStorage', but
...
// ------------------------------------------------------------------------
{
    ...
    cerr << "GET - WITH NO VIRTUAL STORAGES "  << endl;
    ...
    BSLS_ASSERT_OPT(d_tester.addMessages(&guids, storageKeys, k_MSG_COUNT) ==
        ...
        cerr << "guids.size() = " << guids.size() << endl;
